    ...
    public class Stem
    {
        ...
        private void DrawVerticalLine(Graphics g, Pen pen, int ytop, WhiteNote topstaff)
        {
            ...
            else if (direction == Down)
            {
                ...
                if (side == LeftSide)
                    y1 = y1 - SheetMusic.NoteHeight / 4 - 3;
                else
                    y1 = y1 - SheetMusic.NoteHeight / 2 - 3;
        ...
        private void DrawBezier(Graphics g,Pen pen, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4)
        {
            //GraphicsPath path = new GraphicsPath();
            //path.AddBezier(x1, y1-1.0f, x2, y2-0.5f, x3, y3-0.25f, x4, y4);
            //path.AddBezier(x1, y1, x2, y2, x3, y3, x4, y4);
            //path.AddBezier(x1, y1 + 1.0f, x2, y2 + 0.5f, x3, y3 + 0.25f, x4, y4);

            //g.DrawBezier(pen,x1, y1 - 1.0f, x2, y2 - 0.5f, x3, y3 - 0.25f, x4, y4);
